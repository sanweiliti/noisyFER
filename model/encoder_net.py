...
class Encoder(nn.Module):
    def __init__(self, img_size=32, fc_layer=512, latent_dim=10, noise_dim=100):
    ...
    def forward(self, x):
        ...
        # noise z0 + latent code z1
        embedding = self.embedding(conv5_flatten)  # [batch_size, fc_layer]
        mean = self.mean_layer(embedding)
        logvar = self.logvar_layer(embedding)
        z0 = self.sample_z(mean, logvar)
        z1 = self.c_layer(embedding)
        return mean, logvar, z0, z1
